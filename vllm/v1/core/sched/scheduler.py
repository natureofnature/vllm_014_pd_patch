...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Throttle schedule-start logs to avoid spam.
        self._log_last_state: tuple[int, int] | None = None
        self._log_last_ts: float = 0.0
        # Throttle KV waiting logs per request.
        self._kv_wait_last_log: dict[str, float] = {}
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if self.running or len(self.waiting) > 0:
            current_state = (len(self.running), len(self.waiting))
            now = time.monotonic()
            if (
                self._log_last_state != current_state
                or now - self._log_last_ts > 1.0
            ):
                print(
                    "[DECODER][SCHED] schedule start: "
                    f"running={current_state[0]}, waiting={current_state[1]}"
                )
                self._log_last_state = current_state
                self._log_last_ts = now
        ...
        if (
            ...
            print(
                "[DECODER][SCHED] schedule output: "
                f"req_ids={list(num_scheduled_tokens.keys())}, "
                f"tokens={scheduler_output.total_num_scheduled_tokens}, "
                f"preempted={list(scheduler_output.preempted_req_ids)}, "
                f"finished={list(scheduler_output.finished_req_ids)}"
            )
    ...
    def _update_waiting_for_remote_kv(self, request: Request) -> bool:
        ...
        if request.request_id not in self.finished_recving_kv_req_ids:
            now = time.monotonic()
            last_ts = self._kv_wait_last_log.get(request.request_id, 0.0)
            if now - last_ts > 2.0:
                params = request.kv_transfer_params or {}
                finished = list(self.finished_recving_kv_req_ids)
                print(
                    "[SCHED][KV] waiting remote KV: "
                    f"request_id={request.request_id}, "
                    f"remote_request_id={params.get('remote_request_id')}, "
                    f"remote={params.get('remote_host')}:{params.get('remote_port')}, "
                    f"finished_recving={finished}"
                )
                self._kv_wait_last_log[request.request_id] = now
            return False
        ...
        if request.request_id in self._kv_wait_last_log:
            del self._kv_wait_last_log[request.request_id]
        return True
    ...
    def _update_from_kv_xfer_finished(self, kv_connector_output: KVConnectorOutput):
        ...
        * if finished_sending: free the blocks
        ...
        if kv_connector_output.finished_recving:
            print(
                "[SCHED][KV] finished_recving: "
                f"{list(kv_connector_output.finished_recving)}"
            )
