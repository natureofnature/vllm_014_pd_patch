...
class MooncakeConnectorScheduler:
    ...
    def update_state_after_alloc(
        ...
        print(f"[SCHEDULER] update_state_after_alloc: request_id={request.request_id}, "
              f"num_external_tokens={num_external_tokens}, "
              f"kv_transfer_params={params}, kv_role={self.kv_role}")
        logger.debug(
        ...
        if params.get("do_remote_prefill"):
            ...
            if all(p in params for p in ("remote_host", "remote_port")):
                ...
                print(f"[DECODER SCHEDULER] update_state_after_alloc: "
                      f"request_id={request.request_id}, "
                      f"remote_request_id={params.get('remote_request_id')}, "
                      f"remote={params.get('remote_host')}:{params.get('remote_port')}, "
                      f"num_local_blocks={len(local_block_ids)}")
            else:
                logger.warning(
    ...
    def build_connector_meta(
        ...
        if self.kv_role != "kv_producer":
            for req_id, (req, block_ids) in self._reqs_need_recv.items():
                ...
                remote_req_id = req.kv_transfer_params.get(
                    "remote_request_id", req_id
                )
                if remote_req_id != req_id:
                    print(
                        "[DECODER SCHEDULER] build_connector_meta: "
                        f"local_request_id={req_id}, "
                        f"remote_request_id={remote_req_id}, "
...
class MooncakeConnectorWorker:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        # Map remote_request_id -> local_request_id for KV recv completion.
    ...
    async def _mooncake_sender_listener(
        ...
        print(f"[PREFILLER ZMQ] Listening on {path} (hostname={self.hostname}, port={base_port + tp_rank})")
        ...
        try:
            while True:
                ...
                print(f"[PREFILLER ZMQ] Received request: identity_len={len(identity)}, metadata_bytes={len(metadata_bytes)}")
    ...
    async def _sender_worker(self, sock: zmq.asyncio.Socket):
        while True:
            try:
                ...
                try:
                    print(f"[PREFILLER ZMQ] _sender_worker dequeued metadata_bytes={len(metadata_bytes)}")
                    metadata = self._decoder.decode(metadata_bytes)
                    print(f"[PREFILLER ZMQ] Decoded metadata: request_ids={metadata.request_ids}, remote={metadata.remote_hostname}:{metadata.remote_port}")
                    await self.send_kv_to_decode(metadata)
                    await sock.send_multipart((identity, b"", TRANS_DONE))
                    print("[PREFILLER ZMQ] Sent TRANS_DONE reply")
                except Exception as e:
                    logger.error("Error processing Mooncake xfer request: %s", e)
                    await sock.send_multipart((identity, b"", TRANS_ERROR))
                    print(f"[PREFILLER ZMQ] Sent TRANS_ERROR reply: {e}")
                finally:
    ...
    async def send_kv_to_decode(self, meta: MooncakeAgentMetadata):
        print(f"[PREFILLER WORKER] send_kv_to_decode START: request_ids={meta.request_ids}")
        ...
        for req_id in meta.request_ids:
            ...
            if send_meta is None:
                ...
                print(f"[PREFILLER WORKER] send_kv_to_decode FAILED: {req_id} not found in reqs_need_send")
                print(f"[PREFILLER WORKER] Available keys: {list(self.reqs_need_send.keys())}")
                return
            # Mark it as not expired. We will send it now.
            send_meta.expire_time = float("inf")
            send_reqs.append((req_id, send_meta))
            print(f"[PREFILLER WORKER] Found request {req_id}, ready={send_meta.ready.is_set()}, blocks={len(send_meta.local_block_ids)}")

        print(f"[PREFILLER WORKER] Building transfer params...")
        src_ptrs, dst_ptrs, lengths = await self._build_transfer_params(send_reqs, meta)
        print(f"[PREFILLER WORKER] Transfer params built: {len(src_ptrs)} blocks")
        
        remote_session = f"{meta.remote_hostname}:{meta.remote_port}"
        print(f"[PREFILLER WORKER] Starting RDMA transfer to {remote_session}...")
        ...
        if ret_value != 0:
            print(f"[PREFILLER WORKER] RDMA transfer FAILED: {ret_value}")
            raise RuntimeError(f"Error in batch_transfer_sync_write: {ret_value}")

        print(f"[PREFILLER WORKER] RDMA transfer DONE")
        for req_id in meta.request_ids:
    ...
    async def _build_transfer_params(
        ...
        for (req_id, send_meta), remote_block_ids in zip(
            ...
            print(f"[PREFILLER WORKER] Waiting for ready event for {req_id}, is_set={send_meta.ready.is_set()}")
            await send_meta.ready.wait()
            print(f"[PREFILLER WORKER] Ready event received for {req_id}")
    ...
    async def receive_kv(self, path: str, req_blocks: list[tuple[str, list[int]]]):
        ...
        print(f"[DECODER WORKER] receive_kv START: request_ids={req_ids}, path={path}")
        ...
        # Bind to a fixed local port to avoid NAT/firewall issues with replies.
        local_port = self.side_channel_port + 1000 + self.tp_rank
        local_path = make_zmq_path("tcp", self.hostname, local_port)
        sock: zmq.asyncio.Socket = make_zmq_socket(
            self.async_zmq_ctx, local_path, zmq.REQ, bind=True, linger=0
        )
        print(f"[DECODER WORKER] ZMQ bound locally to {local_path}")
        sock.connect(path)
        print(f"[DECODER WORKER] ZMQ connected to {path}")
        sock.setsockopt(zmq.RCVTIMEO, 60000)
        try:
            print(f"[DECODER WORKER] Sending ZMQ request to Prefiller...")
            await sock.send(encoded_data)
            print(f"[DECODER WORKER] ZMQ request sent, waiting for response (timeout=60s)...")
            ret_msg = await sock.recv()
            print(f"[DECODER WORKER] ZMQ response received: {ret_msg}")
            if ret_msg != TRANS_DONE:
                logger.error(
                    "Error happens during tranfering kvcache for %s, see logs in prefiller.",  # noqa: E501
                    req_ids,
                )
                print(f"[DECODER WORKER] receive_kv FAILED: {ret_msg}")
                return
        except zmq.ContextTerminated:
            logger.debug("ZMQ context terminated, exiting Mooncake receiver thread.")
            print(f"[DECODER WORKER] ZMQ context terminated")
        except Exception as e:
            logger.error("MooncakeAgentMetadata transfer failed for %s: %s", req_ids, e)
            print(f"[DECODER WORKER] receive_kv EXCEPTION: {e}")
            return
        finally:
            sock.close()
        print(f"[DECODER WORKER] receive_kv DONE: {req_ids}")
        ...
        for remote_id in req_ids:
            ...
            if local_id is None:
                missing_map.append(remote_id)
                local_req_ids.append(remote_id)
            else:
                local_req_ids.append(local_id)
        if missing_map:
            print(
                "[DECODER WORKER] receive_kv WARN: missing mapping for "
                f"remote_request_id={missing_map}"
            )
        print(
            "[DECODER WORKER] receive_kv DONE mapped: "
            f"local_request_ids={local_req_ids}"
        )
    ...
    def group_kv_pull(self, metadata: MooncakeConnectorMetadata):
        ...
        for req_id, meta in metadata.reqs_to_recv.items():
            ...
            if meta.remote_request_id != req_id:
                print(
                    "[DECODER WORKER] group_kv_pull: "
                    f"local_request_id={req_id}, "
                    f"remote_request_id={meta.remote_request_id}, "
                    f"blocks={len(meta.local_block_ids)}, "
                    f"remote={meta.remote_host}:{meta.remote_port}"
                )
            path = make_zmq_path(
                "tcp", meta.remote_host, meta.remote_port + self.tp_rank
            )
            # Record mapping so recv completion can notify scheduler
            # using the local request_id.
            self.remote_to_local_req[meta.remote_request_id] = req_id
    ...
    async def record_send_reqs(self, metadata: MooncakeConnectorMetadata):
        print(f"[PREFILLER WORKER] record_send_reqs: reqs_to_send={list(metadata.reqs_to_send.keys())}")
        for req_id, block_ids in metadata.reqs_to_send.items():
            if block_ids:
                # Already gone through request_finished()
                print(f"[PREFILLER WORKER] record_send_reqs: {req_id} has {len(block_ids)} blocks, setting ready")
                ...
                print(f"[PREFILLER WORKER] record_send_reqs: {req_id} has empty blocks, creating SendBlockMeta")
    ...
    def start_load_kv(self, metadata: MooncakeConnectorMetadata):
        has_recv = bool(metadata.reqs_to_recv)
        has_send = bool(metadata.reqs_to_send)
        if has_recv or has_send:
            print(
                f"[WORKER] start_load_kv: kv_role={self.kv_role}, "
                f"reqs_to_recv={list(metadata.reqs_to_recv.keys())}, "
                f"reqs_to_send={list(metadata.reqs_to_send.keys())}"
            )

        if self.kv_role != "kv_producer" and has_recv:
            kv_pulls = self.group_kv_pull(metadata)
            print(f"[DECODER WORKER] kv_pulls: {kv_pulls}")
            for path, req_blocks in kv_pulls.items():
                print(f"[DECODER WORKER] Scheduling receive_kv for path={path}, req_blocks={req_blocks}")
                asyncio.run_coroutine_threadsafe(
                    self.receive_kv(path, req_blocks), self.receiver_loop
                )

        if self.kv_role != "kv_consumer" and has_send:
            print(f"[PREFILLER WORKER] Scheduling record_send_reqs...")
            asyncio.run_coroutine_threadsafe(
